import { app, serializeState } from './state.js';
...
export function generateCode() {
	...
	out += `% ${translate('latexGeneratedBy')}\n`;

	const encodedState = serializeState();
	if (encodedState) {
		const currentUrl = window.location.href.split('?')[0];
		out += `% ${translate('latexEditHere')}: ${currentUrl}?data=${encodedState}\n`;
	}
	...
	if (usePreamble && !isObsidian) {
		...
		out += `% ${translate('latexRequiredPackages')}:\n`;
